...
* 10/31/2018
*
* This program reads in assembly language and converts it to object code. ( in is '.s' file,
...
int Assembler::Assemble(fstream& inFile, fstream& outFile)
{
    ...
    while (!inFile.eof())
    {
        ...
        else if (instruction <= 99 && instruction > 9)
        {
            outFile << "000" << instruction << endl;;
        }
        else if (instruction <= 999 && instruction > 99)
        {
            outFile << "00" << instruction << endl;
        }
        else if (instruction <= 9999 && instruction > 999)
        {

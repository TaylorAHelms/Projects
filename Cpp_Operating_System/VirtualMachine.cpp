...
//returns bool if stack overflow occurred
...
//returns bool if stack underflow occurred
...
//returns bool if carry occurred
...
{
    if (val1 == val2 && val1 != (r[rd] & 0x8000) >> 15)
        sr |= 0x10;
    else
        sr &= 0xf;
}
...
void VirtualMachine::store(int& rd, int& i, int& rs, int& addr)
{
    ...
    tlb.page_table[phys / 8].modified = true;
    mem[tlb.log_to_phys(addr)] = r[rd];
}
